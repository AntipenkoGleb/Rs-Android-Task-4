...
class AddCarViewModel(private val repository: BaseCarRepository) : ViewModel() {
    ...
    val viewModelCoroutineContext = viewModelScope.coroutineContext + Dispatchers.IO

    val car = MutableLiveData(Car())

    suspend fun addCar(): Long? {
        return withContext(viewModelCoroutineContext) {repository.insert(car.value!!) }
    }
